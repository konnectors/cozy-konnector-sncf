...
function logIn (requiredFields, entries, data, next) {
  ...
  request(loginOptions, (err, res, body) => {
    ...
    if (jsonRes.error) {
      log('info', `${jsonRes.error.code}: ${jsonRes.error.libelle}`)
      return next('LOGIN_FAILED')
    }
...
function getOrderPage (requiredFields, entries, data, next) {
  ...
  getPage(url, (err, res, body) => {
    ...
    data.html = body
    log('info', 'Orders page downloaded.')
    return next()
  })
}
...
function customFilterExisting (requiredFields, entries, data, next) {
  filterExisting(null, Bill)(requiredFields, entries.bills, data, next)
}

function customSaveDataAndFile (requiredFields, entries, data, next) {
  saveDataAndFile(null, Bill, 'sncf', ['bill'])(requiredFields, entries.bills, data, next)
}
